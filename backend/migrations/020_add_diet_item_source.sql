...
ALTER TABLE diet_items ADD COLUMN IF NOT EXISTS source VARCHAR(50) DEFAULT 'manual';

-- Add index on (pet_id, source) for filtering during analysis and recommendations
CREATE INDEX IF NOT EXISTS idx_diet_items_pet_source ON diet_items(pet_id, source);

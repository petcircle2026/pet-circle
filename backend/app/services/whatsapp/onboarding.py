...
async def _parse_preventive_care(
    ...
    prompt = (
        ...
        "- Return the medicine name EXACTLY as the user wrote it — do NOT expand, "
        "normalize, or add formulation details. "
        "If the user wrote 'bravecto', return 'Bravecto'. "
        "If they wrote 'nexgard spectra', return 'NexGard Spectra'. "
        "Never infer or append words like 'Chew', 'Spot-on', 'Tablet', or weight ranges.\n"
...
async def _step_awaiting_documents(db, user, text_lower, send_fn):
    ...
    if _is_add_more_intent(text_lower):
        try:
            from app.services.whatsapp.message_router import mark_upload_window_extended
...
async def _finalize_onboarding(db, user, send_fn, declined_documents: bool = False):
    ...
    try:
        from app.services.whatsapp.message_router import _clear_deferred_care_plan_marker
    ...
    for master in masters:
        ...
        try:
            ...
            if master.item_name == "Birthday Celebration":
                ...
                from app.services.whatsapp.birthday_service import calculate_next_birthday

...
async def _step_preventive(db, user, text, send_fn):
    ...
    # If the user mentioned a vaccine keyword but GPT returned null (couldn't parse the date),
    # add vaccines to missing so it gets re-asked rather than silently dropped.
    _vax_keywords = {"vaccine", "vaccines", "shot", "shots", "jab", "jabs", "vaccination", "vaccinated"}
    text_lower_check = text.strip().lower()
    if not parsed.get("vaccines") and not (parsed.get("vaccine_specifics") or []):
        if any(kw in text_lower_check for kw in _vax_keywords) and "vaccines" not in missing:
            missing.append("vaccines")
...
async def _parse_diet_input(text: str) -> list[tuple[str, str, str]]:
    ...
    prompt = (
        ...
        "For the 'detail' field: if the message contains a quantity or portion size "
        "for an item (e.g. 100g, 1cup, 2 eggs, 50ml), extract it as the detail. "
        "The message may contain a 'Portions:' section that maps items to quantities — "
        "pair each item with its quantity. If no quantity is given for an item, use \"\".\n\n"
        "Examples:\n"
        ...
        "  Input: \"chicken eggs curd. Portions: eggs 100g curd 1cup\"\n"
        "  Output: [{\"label\":\"chicken\",\"detail\":\"\",\"kind\":\"ingredient\"}, "
        "{\"label\":\"eggs\",\"detail\":\"100g\",\"kind\":\"ingredient\"}, "
        "{\"label\":\"curd\",\"detail\":\"1cup\",\"kind\":\"ingredient\"}]\n"
        "  Input: \"boiled chicken 150g and carrots 50g\"\n"
        "  Output: [{\"label\":\"boiled chicken\",\"detail\":\"150g\",\"kind\":\"ingredient\"}, "
        "{\"label\":\"carrots\",\"detail\":\"50g\",\"kind\":\"ingredient\"}]\n"

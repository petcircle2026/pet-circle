...
EXTRACTION_SYSTEM_PROMPT = (
    ...
    "    DEWORMING SECTION RULE: In a clearly labelled 'Deworming' section, EVERY numbered or "
    "bulleted dated entry is a deworming administration — even if it has only a checkmark (✓), "
    "a weight note (e.g. 'weight.'), a dosage ('1½ tab'), or no medication name at all. "
    "Extract EACH as item_name='Deworming' with the entry's date as last_done_date. "
    "Do NOT skip entries just because no drug name or dosage is written next to them.\n"
    "    TICK/FLEA SECTION RULE: In a clearly labelled 'Tick/Flea', 'Flea & Tick', or "
    "'Anti-parasitic' section, EVERY numbered or bulleted dated entry is a tick/flea "
    "administration — even if it has only a checkmark (✓), a dosage, or no product name. "
    "Extract EACH as item_name='Tick/Flea' with the entry's date as last_done_date. "
    "Do NOT skip entries just because no product name is written next to them.\n"

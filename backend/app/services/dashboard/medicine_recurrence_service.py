...
def _lookup_catalog_frequency(db: Session, medicine_name: str, item_type: str) -> int | None:
    ...
    Queries by category (flea_tick/deworming) then matches medicine name,
    returning the minimum interval (shortest for dual-use medicines).
    Falls through to GPT when no catalog match is found.
    """
    ...
    try:
        ...
        if is_dual:
            categories = ["flea_tick", "deworming"]
        else:
        ...
        rows = db.query(
            ProductMedicines.type,
            ProductMedicines.brand_name,
            ProductMedicines.product_name,
            ProductMedicines.repeat_frequency,
        ).filter(
            ProductMedicines.type.in_(categories)
        ).all()

        name_lower = medicine_name.lower()
        matching_days: list[int] = []
        for row in rows:
            _, brand, prod_name, freq = row
            full_name = f"{brand} {prod_name}".lower()
            if name_lower in full_name or any(part in full_name for part in name_lower.split()):
                days = _parse_frequency_to_days(freq)
                if days:

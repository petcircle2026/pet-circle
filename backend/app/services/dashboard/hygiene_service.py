...
from app.repositories.hygiene_preference_repository import HygienePreferenceRepository
...
async def _generate_hygiene_tips(
    ...
    hygiene_repo = HygienePreferenceRepository(db)
    tips = hygiene_repo.find_tip_cache(species, breed_norm, item_ids, cutoff)
    ...
    try:
        ...
        if new_tips:
            ...
            hygiene_repo.delete_stale_tip_cache(species, breed_norm, stale_ids)
            new_cache_rows = [
                HygieneTipCache(
                    species=species,
            ...
            hygiene_repo.bulk_insert_tips(new_cache_rows)
            db.commit()
...
async def get_hygiene_preferences(
    ...
    """
    hygiene_repo = HygienePreferenceRepository(db)
    prefs = hygiene_repo.find_by_pet(pet_id)
...
async def add_hygiene_item(
    ...
    existing = hygiene_repo.find_by_pet_and_item(pet_id, item_id)
    if existing:
...
async def upsert_hygiene_preference(
    ...
    hygiene_repo = HygienePreferenceRepository(db)
    pref = hygiene_repo.find_by_pet_and_item(pet_id, item_id)
...
async def delete_hygiene_item(db: Session, pet_id, item_id: str) -> dict:
    ...
    hygiene_repo.delete(pet_id, item_id)
    db.commit()

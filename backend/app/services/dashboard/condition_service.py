...
async def get_condition_timeline(db: Session, pet_id: UUID) -> dict:
    ...
    conditions = care_repo.find_active_conditions_with_relations(pet_id)
    ...
    preventive_rows = [r for r in preventive_repo.get_by_pet(pet_id) if r.last_done_date]
...
def update_condition(db: Session, pet_id: UUID, condition_id: UUID, updates: dict) -> dict:
    ...
    condition = health_repo.get_condition_by_id_and_pet(condition_id, pet_id)
    if not condition:
...
def update_condition_medication(db: Session, pet_id: UUID, medication_id: UUID, updates: dict) -> dict:
    ...
    health_repo = HealthRepository(db)
    med = health_repo.get_medication_by_id_for_pet(medication_id, pet_id)
    if not med:
...
def update_condition_monitoring(db: Session, pet_id: UUID, monitoring_id: UUID, updates: dict) -> dict:
    ...
    health_repo = HealthRepository(db)
    mon = health_repo.get_monitoring_by_id_for_pet(monitoring_id, pet_id)
    if not mon:
...
async def get_condition_recommendations(db: Session, pet_id: UUID) -> dict:
    ...
    preventive_repo = PreventiveRepository(db)
    preventive_rows = preventive_repo.get_by_pet(pet_id)
    ...
    for cond in conditions:
        ...
        for mon in cond.monitoring:
            if mon.next_due_date and mon.next_due_date < today:
                ...
                priority = "urgent" if days_overdue > URGENT_OVERDUE_DAYS else "high"
        ...
        for med in cond.medications:
            if is_medication_active(med, cond) and med.refill_due_date:
        ...
        # Chronic and recurrent conditions without monitoring checks set up
        if cond.condition_type in {"chronic", "recurrent"} and len(cond.monitoring) == 0:
            recommendations.append({
                "icon": "📋",

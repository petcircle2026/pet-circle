...
"""
from app.models import IdealWeightCache
...
from app.repositories.weight_history_repository import WeightHistoryRepository
from app.repositories.ideal_weight_cache_repository import IdealWeightCacheRepository
...
async def get_ideal_range(
    ...
    # 1. Check DB cache
    try:
        cache_repo = IdealWeightCacheRepository(db)
        cached = cache_repo.find_by_criteria(
            species_normalized, breed_normalized, gender_normalized, age_category
        )
...
async def get_weight_history(db: Session, pet_id, pet: Pet) -> dict:
    ...
    entries = weight_repo.find_by_pet(pet_id)

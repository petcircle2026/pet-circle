...
async def get_dashboard_data(db: Session, token: str) -> dict:
    ...
    pet = PetRepository(db).get_by_id_with_user(pet_id)
    ...
    def _fetch_conditions_sync():
        own_db = SessionLocal()
        try:
            rows = ConditionRepository(own_db).find_displayable_conditions(pet_id)
    ...
    def _fetch_diagnostics_sync():
        own_db = SessionLocal()
        try:
            rows = HealthRepository(own_db).get_diagnostics_by_pet(pet_id)
            return [
                {
    ...
    preventive_records_list = PreventiveRepository(db).get_by_pet_with_master_for_dashboard(pet_id)
...
async def get_pet_photo_for_token(
    ...
    pet = PetRepository(db).get_by_id(dashboard_token.pet_id)
...
def get_health_trends(db: Session, token: str) -> dict:
    ...
    preventive_records_list = PreventiveRepository(db).get_by_pet_with_master(pet_id)

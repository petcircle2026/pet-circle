...
from app.repositories.diagnostic_test_result_repository import DiagnosticTestResultRepository
...
def _fetch_diagnostic_results_for_documents(
    ...
    if not document_ids:
        return {}
    diagnostic_repo = DiagnosticTestResultRepository(db)
    return diagnostic_repo.find_by_document_ids(pet_id, document_ids)
...
def _fetch_documents(db: Session, pet_id: Any) -> list[Document]:
    ...
    return doc_repo.find_successful_documents(pet_id)
...
def _fetch_failed_documents(db: Session, pet_id: Any) -> list[Document]:
    ...
    """
    doc_repo = DocumentRepository(db)
    return doc_repo.find_failed_documents(pet_id)
...
def _fetch_conditions_for_documents(
    ...
    rows = health_repo.find_active_conditions_with_relations(pet_id, document_ids)

...
from app.services.dashboard.weight_service import DEFAULT_RANGE as DEFAULT_IDEAL_WEIGHT_RANGE
from app.services.dashboard.weight_service import get_ideal_range
...
async def get_nutrition_targets(
    ...
    # 1. Check DB cache
    try:
        nutrition_cache_repo = NutritionCacheRepository(db)
        cached = nutrition_cache_repo.find_nutrition_target_cache(
...
async def estimate_complete_meal_nutrition(
    ...
    try:
        nutrition_cache_repo = NutritionCacheRepository(db)
        cached = nutrition_cache_repo.find_food_nutrition_cache_by_label_and_type(cache_key, food_type)
        if cached:
...
async def analyze_nutrition(db: Session, pet_id) -> dict:
    ...
    conditions = condition_repo.find_by_pet_and_active(pet_id)
    ...
    diet_items = diet_repo.find_by_pet_id(pet_id)

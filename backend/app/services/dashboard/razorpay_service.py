...
from app.core.constants import RAZORPAY_RETRY_BACKOFFS
...
# Razorpay error strings that indicate a transient failure worth retrying.
_RAZORPAY_RETRYABLE_MSGS = ("timeout", "connection", "503", "502", "500", "network")
...
def _is_razorpay_retryable(exc: Exception) -> bool:
    """Return True for transient Razorpay errors (network, 5xx). False for 4xx logic errors."""
    exc_str = str(exc).lower()
    return any(kw in exc_str for kw in _RAZORPAY_RETRYABLE_MSGS)


async def _razorpay_call_with_retry(func, *args, **kwargs):
    """
    Execute a synchronous Razorpay SDK call in a thread pool with retry.

    Retries up to len(RAZORPAY_RETRY_BACKOFFS) times on transient errors
    (network timeouts, 5xx responses). Permanent errors (4xx, bad credentials)
    are re-raised immediately without retry.

    Args:
        func: Synchronous callable (e.g. client.order.create).
    ...
    total_attempts = 1 + len(RAZORPAY_RETRY_BACKOFFS)
    ...
    for attempt in range(total_attempts):
        try:
            ...
            if is_last or not _is_razorpay_retryable(exc):
                logger.error(
                    "Razorpay call failed (attempt %d/%d, permanent=%s): %s",
                    attempt + 1, total_attempts, not _is_razorpay_retryable(exc), exc,
                )
                raise exc
            backoff = RAZORPAY_RETRY_BACKOFFS[attempt]
            logger.warning(
                "Razorpay call failed (attempt %d/%d, transient), retrying in %ss: %s",
                attempt + 1, total_attempts, backoff, exc,
            )
...
async def create_razorpay_payment(
    ...
    if client_items:
        ...
        in_cart = cart_repo.find_in_cart_by_pet(pet_id)
        if not in_cart:
    ...
    # Create Razorpay order with transient-error retry.
    client = _get_razorpay_client()
    rzp_order = await _razorpay_call_with_retry(
        client.order.create,
        {
            "amount": amount_paise,
            "currency": "INR",
...
async def verify_razorpay_payment(
    ...
    order = order_repo.find_by_id(order_db_id)
    if not order:

"""
PetCircle Phase 1 â€” Central Constants
...
# Internal cap only â€” do not surface this number in any user-facing message or LLM prompt.
...
# Peak load: 15 uploads + 8 extractions = 23 sessions â€” within pool ceiling of 25.
...
# ~90KB â€” negligible. Above this we're likely under a DoS pattern, not real use.
MAX_QUEUED_UPLOADS: int = 30

# Maximum number of concurrent background message processing tasks system-wide.
# This is the ENTRY-POINT semaphore â€” it gates every message type (text, button,
...
# handlers alone â€” they hold sessions briefly and release them (not the full
...
# 30 active + 70 queued = 100 max coroutines â€” covers all 100 concurrent users.
...
# This is a system-wide constant â€” never use UTC or other zones.
...
# Never hardcode these strings elsewhere â€” always reference these constants.
REMINDER_DONE: str = "REMINDER_DONE"            # "Done â€” Log It" (Due stage)
...
REMINDER_ORDER_NOW: str = "REMINDER_ORDER_NOW"  # "Order Now" (Due stage) â†’ agentic_order
REMINDER_STILL_PENDING: str = "REMINDER_STILL_PENDING"  # "Still Pending" (D+3 / Overdue)
REMINDER_SCHEDULE: str = "REMINDER_SCHEDULE"    # "Schedule For ()" â†’ awaiting_reschedule_date
...
# All currently set to 7 days â€” change individual values to configure.
...
# =============================================================================
# AI MODEL SWITCH â€” change only AI_PROVIDER to swap provider/tier everywhere.
...
# --- Active models â€” resolved from AI_PROVIDER (do not edit below this line) ---
...
# Aliases for existing imports â€” these resolve to the models above based on AI_PROVIDER
OPENAI_EXTRACTION_MODEL: str = AI_EXTRACTION_MODEL
OPENAI_QUERY_MODEL: str = AI_QUERY_MODEL

# Temperature for extraction â€” deterministic output required.
...
# Query-specific settings â€” separated from extraction to allow independent tuning.
...
# Rate-limit (429) specific backoffs â€” longer to allow TPM window recovery.
...
# The model rates its own confidence 0.0â€“1.0; values below this floor mean
...
# WhatsApp retries â€” single retry only.
...
# Enforced in the upload service â€” never construct paths manually elsewhere.
...
# Template Quick Reply buttons have fixed payloads â€” no order_id in payload.
...
# Map button payload â†’ database category value.
...
# Map database category â†’ display label for WhatsApp messages.
...
# Source sort order â€” record-based nudges rank above AI-generated ones.
...
# Nudge cache freshness â€” skip regeneration if nudges are younger than this.
NUDGE_CACHE_HOURS: int = 6

# --- Nudge Scheduler â€” Level System ---
...
# Level 2 data priority for Breed + Data nudge sequencing (slots 1â€“3).
...
# The 5 primary report types (Blood Report â†’ PCR) map to the PDF appendix sections.
